...
class KafkaTinyDBSync:
    ...
    def process_message(self, message):
        ...
        try:
            ...
            # 從消息值中提取操作類型和數據
            operation = message.value.get('op')  # c=insert, u=update, d=delete
            data = message.value.get('after')  # 對於插入和更新操作
            before_data = message.value.get('before')  # 對於刪除操作
            ...
            # 根據操作類型處理
            if operation == 'c':  # 插入
                self._handle_insert(table, data)
            elif operation == 'u':  # 更新
                self._handle_update(table, data)
            elif operation == 'd':  # 刪除
